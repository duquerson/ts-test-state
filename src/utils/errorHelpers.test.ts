...
describe('errorHelpers', () => {
	...
	it('should return the fallback message if statusCode is unknown and no message exists', () => {
		// Include an empty string for the message property to satisfy the type
		const errorWithUnknownStatus: FetchError = { statusCode: 503, message: '' }
		expect(getQuizErrorMessage(errorWithUnknownStatus)).toBe('An unexpected error occurred.')

		const errorWithoutStatusOrMessage: FetchError = {
			statusCode: 0,
			message: ''
		}
